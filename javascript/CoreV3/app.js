...
if (urlParams.has('ip')) {_url=urlParams.get('ip');}
else {
	_url = location.href;
    _url += (_url.split('?')[1] ? '&':'?') + "ip=192.168.0.138";
	location.href=_url; //add the default ip address and reload the page
	
}
console.log(_url);
// true
//console.log(urlParams.get('ip')); // "edit"
//console.log(urlParams.getAll('ip')); // ["edit"]
//console.log(urlParams.toString()); // "?post=1234&action=edit"
//console.log(urlParams.append('active', '1')); // "?post=1234&action=edit&active=1"	<--	this one fails, dont need it anyway
		
//location.href+=(location.href.split('?')[1] ? '&':'?') + "param"; //<--works but ned to be careful as it reloads the page
...
apple.mask = thing;
	...
	bikeInputs=new ESPdata("http://"+_url+"/Data");

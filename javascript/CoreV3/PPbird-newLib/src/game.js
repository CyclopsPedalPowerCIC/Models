...
var PILESPEED = 100; //added by woody to slow down piles...
...
function animate() {
	...
	if(Begin){
			...
			var accel=0//(opener.bikeInputs.data(2));
			//console.log(accel);
	...
	else{ //here we add all the lose conditions
	   if((bird.position.y + bird.height/2) > SKYHEIGHT){
	   		//hit()
			bird.position.y= SKYHEIGHT-bird.height/2;
			bird.speedY=0; //hits ground (cycles along!)
	   }
	   
	   if((bird.position.y) < 0){
	   		hit();  //hits ceiling, dies
	   }

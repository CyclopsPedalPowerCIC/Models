...
function line_checkMaxMin(chart, heights, length, start){
	...
	mm=[heights[0][start],heights[1][start],heights[0][start],heights[1][start]]; //minx miny maxx maxy
...
function line_change(chart,heights,length, start){
	...
	chart.line.moveTo((heights[0][start]-chart.minx)*(chart.size[3]-chart.size[1])/(chart.maxx-chart.minx),(-heights[1][start]-chart.miny)*(chart.size[2]-chart.size[0])/(chart.maxy-chart.miny))

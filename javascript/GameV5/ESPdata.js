...
function ESPdata(url){
	...
	this.initialised=function(){if(((this.html[0]&&this.html[1])=="")||((this.html[0]&&this.html[1])==undefined)){return false;}else {return true;}};
	this.namesens=["",""];//array to store names of each sensor
	this.detect=function(){detectSensor	s(this);};
	this.data=function(number) {return getData(this, number);};
...
function getData(espdata,number){ //gets data already downloaded data string
	var tempc=0;
	var rString="";
	var loc=0;
	...
	loc=(espdata.html[tempc]).split("\n")[number].split(":");
	//console.log(loc);
	rString = loc[loc.length-1]; //removes non numeric elements
	//console.log(rString);
	return parseFloat(rString);
}

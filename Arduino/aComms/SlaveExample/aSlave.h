...
void aHost::reply(){
  ...
  Serial.print(ret);
  if (ret.length()>messageLength){Serial.print("Warning, message too long!"};
  for (int i=ret.length();i<messageLength;i++){Serial.print('-');}//to ensure the returned data is always equal to the character limit
  Serial.print('\n');

...
class aComms{
  public:
    ...
    sendRequest(aDevice*,char,char,void*); //includes option to run a function when the data is recieved
...
aComms::sendRequest(aDevice *device,char request1,char request2,void *inputFunction){
    (*device).state=0;
    mState[currentID]=0;//set state ready for recieve
    devices[currentID]=device;

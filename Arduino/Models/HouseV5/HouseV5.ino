...
int findSocket(byte addr[8]){
   ...
   for (int i=0;i<6;i++){
    ...
    for (int j=0;j<pow2(5-i);j++){
      socketSet(j+lowerbound, 1);  //turn on first half of next sockets
    }
  ...
  socketstate[lowerbound]=1;  //register the device as on
  return(lowerbound);
...
//  delay(500);
 ...
 // uint8_t pinValues[] = { B11010100 }; 
 // sockets.setAll(pinValues); 
 // delay(1000);

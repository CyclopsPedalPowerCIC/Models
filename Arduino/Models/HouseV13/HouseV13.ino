/*
  rewrote much of the code after events at park lane college, working as of 16/5/18
*/
...
int unq2=7;  //as unique address parts that are saved to check with device is conected
...
int Skitchen=0;  //mapping which socket block goes first, not actually used?
int Sdining =1;
int Slounge =2;
int Sbed1   =3;
int Sbed2   =4;
...
void loop() {
  ...
  for (int i=0;i<6;i++){
  comms.sendRequest(&control[i],'S','?',&setCon);  //send request
  delay(1); //delay to reduce chance of messages colliding
  comms.check();  //check if a message has been recieved
  }
...
void setCon(){  //sets the controlState array according to the states that have been read form the physical control boards

...
int lights[] = {0,4,4,12,6,4,4,8,8,0};  //order: lounge,dining,kitchen,bath,hall,bed3,bed2,bed1, (loft, not yet)(lounge shold be 8 but had to remove one dodgy set
...
void set1(){
  ...
  for (int j=0;j<7;j++){
    if ((bool(control1.returnString[j]-'0'))==1){
       setDevice(linkList[j],1);
    }
    else if ((bool(control1.returnString[j]-'0'))==0){
...
void set2(){
  ...
  for (int j=0;j<7;j++){
    if ((bool(control2.returnString[j]-'0'))==1){
       setDevice(linkList[7+j],1);
    }
    else if ((bool(control2.returnString[j]-'0'))==0){
...
void set3(){
for (int j=0;j<7;j++){
    if ((bool(control3.returnString[j]-'0'))==1){
       setDevice(linkList[14+j],1);
    }
    else if ((bool(control3.returnString[j]-'0'))==0){
...
void set4(){
  for (int j=0;j<7;j++){
    if ((bool(control4.returnString[j]-'0'))==1){
       setDevice(linkList[21+j],1);
    }
    else if ((bool(control4.returnString[j]-'0'))==0){
...
void set5(){
  for (int j=0;j<7;j++){
    if ((bool(control5.returnString[j]-'0'))==1){
       setDevice(linkList[28+j],1);
    }
    else if ((bool(control5.returnString[j]-'0'))==0){
...
void set6(){
  for (int j=0;j<7;j++){
    if ((bool(control6.returnString[j]-'0'))==1){
       setDevice(linkList[35+j],1);
    }
    else if ((bool(control6.returnString[j]-'0'))==0){
...
void setDevice(int device,int state){
  ...
  if ((device>-1)&&(device<NUM_SOCKETS)){sockets.setNoUpdate(device, state);
  }
...
void setLights(){
  ...
  for (int j=1;j<8;j++){
    ...
    if ((bool(control1.returnString[j-1]-'0'))==1){
    setRoom(j,Brightness);
    }
    else if ((bool(control1.returnString[j-1]-'0'))==0){setRoom(j,0);}
  }

//version of the code to work without a shift register, all but the first arduino made
...
int capCPin = 13;
int threshold=1500;  //may need to be attached to a potentiometer to adjust depending on the weather
...
long start = millis();
int mode[totalPins]={1,1,1,1,1,1,1}; //0 = timed or basic (if t=0) 1= toggle 2=off, 3=on
long timer[totalPins]={0,00,0000,0,0,0,0}; //time to stay on for after reset or toggle state!
...
for (int i = 0; i < totalPins; i++) {
   ...
   dataLink.addFunc('?','C',&setCapacitance);  //set Delay Timer aIDMD?MPTTTT  where P is pin and T is Timer * 0.1 seconds
}
...
void setCapacitance(){
  ...
  //.... need to add code to make this change the threshold of the control board
  }
...
void setTimer(){
  ...
  if (mode[pin]!=0){mode[pin]=0;} //put mode to timer mode
  timer[pin]=0;
  int digits=0;

...
int controlState[NUM_SOCKETS]; //plus 38 always on, 39 always off
...
for (int i=0;i<80;i++){
  linkList[i]==99;  //set all to disconnected
}
...
void loop() {
  comms.sendRequest(&control1,'S','?',&set1);  //sedn request
  ...
  //Serial.print("loop");
 delay(40);  //delay beacuse this loop doesn't need to happen at lightning speed, may cause comms issues if it does, maybe?
  //comms.check();
  ...
  //checkSockets();
   basicFind();   //also sets all of the data for the sockets  this line slows doesn everything else, only run every x times?
...
void basicFind(){
  ...
  while ( plugs.search(addr)) {
   ...
   linkList[i]=38;  //set to always on to start with

  }
  if (i==0){
    socketAddress[0][i]=false;
   socketAddress[1][i]=false;
   linkList[i]=99;   //may want to be changed later?
    }
  }
}
...
int outByte2 = 0;
for(int i=0; i<8; i++){
   outByte1 |= bool(controlState[70+i]) << i;
}
for(int i=0; i<numLights-8; i++){
   outByte2 |= bool(controlState[78+i]) << i;
}

...
void aComms::sendRequest(aDevice *device,char request1,char request2){
    ...
    for (int i=0;(i<messageLength-4);i++){
      Serial.print('-');}
     int parityBit=char((int((*device).deviceID[0])+int((*device).deviceID[1])+int(mID[0][currentID])+int(mID[1][currentID])+int(request1)+int(request2)+int('-')+int('-')+int('-')+int('-')+int('-'))%256);

//this is the version fixed for ESP's !
...
class aHost{
  ...
  private:
    ...
    aHost::aHost(){
     ...
     // EEPROM.write(0,char('T'));
     // EEPROM.write(1,char('T'));  //for first upload if character assigned isn't easy to handle
...
void aHost::getData()
{
  ...
  while (Serial.available() > 0) 
  {
    ...
    {   
      ...
      for(int i = 0; i<(messageLength+3);i++)  // Read in the next X chars - this is the data
      {
